...
impl<T: Namespace + ?Sized> Generate<T> for ty::Param<T> {
...
impl<T: Namespace + ?Sized> GenerateIn<T> for Type<T> {
...
impl<T: Namespace + ?Sized> Generate<T> for ty::Variant<T> {
	fn generate(&self, context: &Context<T>) -> TokenStream {
...
impl<T: Namespace + ?Sized> Generate<T> for ty::Ref {
	fn generate(&self, context: &Context<T>) -> TokenStream {
		super::path(context, context.type_path(*self).unwrap())
	}
}

impl<T: Namespace + ?Sized> Generate<T> for Box<ty::Expr<T>> {
	fn generate(&self, context: &Context<T>) -> TokenStream {
		self.as_ref().generate(context)
	}
}

impl<T: Namespace + ?Sized> Generate<T> for ty::Expr<T> {
	fn generate(&self, context: &Context<T>) -> TokenStream {
		match self {
