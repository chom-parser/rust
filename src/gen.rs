...
fn path<T: Namespace>(context: &Context<T>, path: chom_ir::Path<'_, T>) -> TokenStream {
	...
	for segment in path {
		...
		let id = match segment {
			...
			Segment::Function(id, sig) => function_id(context, id, sig)
		};
...
fn type_id<T: Namespace>(context: &Context<T>, id: chom_ir::ty::Id<T>) -> TokenStream {
	match id {
		chom_ir::ty::Id::Native(n) => {
			...
			match n {
				Native::Reference => quote! { & },
				Native::Unit => quote! { () },
				Native::Char => quote! { char },
				Native::String => quote! { &str },
				...
				Native::Stream => quote! { Iterator },
				Native::Output => quote! { fmt::Formatter }
			}
		},
