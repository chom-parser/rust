...
pub trait Generate<T: Namespace + ?Sized> {
	fn generate(&self, context: &Context<T>) -> TokenStream;
}

/// Generate code inside a scope.
pub trait GenerateIn<T: Namespace + ?Sized> {
	fn generate_in(&self, context: &Context<T>, scope: Scope<T>) -> TokenStream;
}

fn path<T: Namespace + ?Sized>(context: &Context<T>, path: chom_ir::Path<'_, T>) -> TokenStream {
...
fn module_id<T: Namespace + ?Sized>(context: &Context<T>, id: &chom_ir::module::Id<T>) -> TokenStream {
...
fn type_id<T: Namespace + ?Sized>(context: &Context<T>, id: chom_ir::ty::Id<T>) -> TokenStream {
	match id {
...
fn param_id<T: Namespace + ?Sized>(context: &Context<T>, id: T::Param) -> TokenStream {
	let ident = context.id().param_ident(id);
	let id = quote::format_ident!("{}", ident.to_caml_case());
	quote! { #id }
}

fn field_id<T: Namespace + ?Sized>(context: &Context<T>, id: T::Field) -> TokenStream {
	let ident = context.id().field_ident(id);
	let id = quote::format_ident!("{}", ident.to_snake_case());
	quote! { #id }
}

fn variant_id<T: Namespace + ?Sized>(context: &Context<T>, v: &chom_ir::ty::Variant<T>) -> TokenStream {
...
fn function_id<T: Namespace + ?Sized>(context: &Context<T>, sig: &chom_ir::function::Signature<T>) -> TokenStream {
...
fn type_expr_ident<T: Namespace + ?Sized>(context: &Context<T>, e: &chom_ir::ty::Expr<T>) -> chom_ir::Ident {
	use chom_ir::ty::Expr;
	match e {
...
fn var_id<T: Namespace + ?Sized>(context: &Context<T>, id: T::Var) -> TokenStream {
	let ident = context.id().var_ident(id);
	let id = quote::format_ident!("{}", ident.to_snake_case());
	quote! { #id }
}

fn label_id<T: Namespace + ?Sized>(context: &Context<T>, id: T::Label) -> TokenStream {
